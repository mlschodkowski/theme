...
built-in syntax theme IDs, so its import uses `one` (the app's One Dark Pro
variant) as the closest supported syntax base while keeping MLS surfaces,
accents, and diff colors.
